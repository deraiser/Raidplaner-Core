        ...
        <table data-type="info.daries.rp.character" class="table jsClipboardContainer jsObjectActionContainer" data-object-action-class-name="rp\data\character\CharacterAction">
            <thead>
                <tr>
                    ...
                    <th class="columnText columnCharacterName{if $sortField == 'characterName'} active {@$sortOrder}{/if}"><a href="{link controller='CharacterList' application='rp' id=$searchID}pageNo={@$pageNo}&sortField=characterName&sortOrder={if $sortField == 'characterName' && $sortOrder == 'ASC'}DESC{else}ASC{/if}{@$linkParameters}{/link}">{lang}rp.character.name{/lang}</a></th>
